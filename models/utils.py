...
class ContrastiveEmbed(nn.Module):
    def __init__(self, max_support_len=256):
        """
        Args:
    ...
    def forward(self, x, support_dict):
        ...
        y = support_dict["encoded_support"]
        support_token_mask = support_dict["support_token_mask"]

        res = x @ y.transpose(-1, -2)
        res.masked_fill_(~support_token_mask[:, None, :], float("-inf"))

        # padding to max_support_len
        new_res = torch.full((*res.shape[:-1], self.max_support_len), float("-inf"), device=res.device)
