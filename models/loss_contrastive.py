...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # 创建新的目标张量，形状为 (4, 900, 2)，初始化为0
        new_logits = torch.zeros(4, 900, 2)

        # 遍历每个批次和查询，将目标类别合并到新的 logits 中
        for batch in range(src_logits.shape[0]):
            for query in range(src_logits.shape[1]):
                new_logits[batch, query, 1] = sum(src_logits[batch, query, targets[batch]['labels']])
                #0这个位置等于除了目标类别之外的所有类别的和
                new_logits[batch, query, 0] = sum(src_logits[batch, query, :]) - new_logits[batch, query, 1]
        # 现在 new_logits 形状为 (4, 900, 2)，其中第一个类别包含了背景类别信息，第二个类别包含了目标类别信息
        ...
        #loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
        loss_ce = F.cross_entropy(new_logits.transpose(1, 2), target_classes, self.empty_weight)
        # todo
