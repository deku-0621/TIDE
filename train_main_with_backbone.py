...
from engine_with_backbone import train_one_epoch_only
from model_with_backbone import build_model,build_loss
from dataset.mydataset_backbone import MyDataset,collate_fn_TIDE
...
def get_args_parser():
    ...
    parser.add_argument('--num_workers', default=0, type=int)#todo test
    ...
    parser.add_argument("--pretrained_transfer",default=True,type=bool)
...
def main(args):
    ...
    if args.pretrained_transfer and not args.resume:
        pretrain_path = 'train_output/checkpoint_20230910_N2_CLS2BOX5GIOU2_Q900_backbone_embedding.pth'
        pretrain_model = torch.load(pretrain_path)
        pretrained_dict = pretrain_model['model']
        model_dict = model.state_dict()
        # match layer state and shape
        pretrained_dict = {k: v for k, v in pretrained_dict.items() 
                        if k in model_dict and v.shape == model_dict[k].shape}
                        #and 'fusion' not in k 
                        #and 'ca_support' not in k 
                        #and 'cross' not in k}
        # update state dict
    ...
    #todo
    dataset_train = MyDataset(train_config.train_ann_path, train_config.train_image_folder,train=True)
    #dataset_train = MyDataset(train_config.train_ann_path, train_config.train_image_folder,train_config.support_feats_folder,train_config.swin_feats_folder, train=True)
    if args.distributed:
